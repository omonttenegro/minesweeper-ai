...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        while changed:
            ...
            # Apply deductions for every sentence
            for sentence in list(self.knowledge):
                for mine in sentence.known_mines() or set():
                    if mine not in self.mines:
                        self.mark_mine(mine)
                        changed = True
                for safe in sentence.known_safes() or set():
                ...
                for j in range(i + 1, len(self.knowledge)):
                    ...
                    if a.cells < b.cells:
                        ...
                        if diff_cells and diff_count >= 0:
                            nova = Sentence(diff_cells, diff_count)
                            if nova not in self.knowledge and nova not in novas:
                                novas.append(nova)
                    elif b.cells < a.cells:
                        diff_cells = a.cells - b.cells
                        diff_count = a.count - b.count
                        if diff_cells and diff_count >= 0:
                            nova = Sentence(diff_cells, diff_count)
                            if nova not in self.knowledge and nova not in novas:
                                novas.append(nova)

            # check mines & safes from novas
            if novas:
                self.knowledge.extend(novas)
                for sentence in novas:
                    for mine in sentence.known_mines() or set():
                        if mine not in self.mines:
                            self.mark_mine(mine)
                            changed = True
